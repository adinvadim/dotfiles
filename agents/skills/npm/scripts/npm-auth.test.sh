...
ITEM_JSON='{"fields":[{"id":"username","value":"owner"},{"id":"password","value":"secret"}]}'
npm_auth_whoami() {
  return 1
}
current_otp() {
...
if ensure_npm_auth 2>"$TEST_ROOT/preflight-error.log"; then
  echo "missing TOTP preflight unexpectedly succeeded" >&2
  exit 1
else
  test "$?" -eq 3
fi
grep -Fq "no working registry_token or six-digit TOTP field" "$TEST_ROOT/preflight-error.log"
